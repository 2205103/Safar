...
router.post('/', checkToken, async (req, res) => {
  ...
  const { user_id, total_cost } = abc.rows[0];
  ...
  try {
    const result = await client.query(`
      SELECT 
        ...
        tt.arrival_time,
        seat_number,
        (sr.date + tt.arrival_time - CURRENT_TIMESTAMP) AS time_remaining
    ...
    const { date, train_code, train_name, from_station_name, to_station_name, arrival_time, time_remaining } = result.rows[0];
    ...
    res.json({
      ...
      time_remaining: time_remaining, // added this
